...
public class DeleteVisitor implements Visitor<Object> {
  ...
  public Object visit(SqlHList list) {
    ...
    try {
      ...
      SqlHBaseList.listeners.inform(list.dao);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // commits changes
          deleteObjectOfList(object, atomic);
  ...
  private void deleteListOfObject(SqlHBaseList<?> list, Atomic atomic, int iid) throws SQLException {
    DaoAttlist attParent = db.select(DaoAttlist.class).where("value = ?", iid).first();
  ...
  private void deleteObjectOfList(SqlHObject object, Atomic atomic) throws SQLException {
    DaoLsobj lsParent = db.select(DaoLsobj.class).where("value = ?", object.dao.getId()).first();
  ...
  private void deleteObjectOfObject(SqlHObject object, Atomic atomic, int iid) throws SQLException {
    DaoAttobj attParent = db.select(DaoAttobj.class).where("value = ?", iid).first();
    DaoObject objParent = db.select(DaoObject.class, attParent.getParent());
