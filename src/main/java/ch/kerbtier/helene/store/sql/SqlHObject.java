...
import ch.kerbtier.struwwel.DoubleMappedObservable;
...
public class SqlHObject extends SqlHNode implements HObject, EntitySubject {
  ...
  private static DoubleMappedObservable<DaoObject, String> changeListeners = new DoubleMappedObservable<>();
  ...
  protected void createRaw(Db db, Map<String, Object> data) throws SQLException {
    for (String field : data.keySet()) {
      ...
      if (value == null) {
        try {
          Object attr = db.select(daodef).where("parent = ? and name = ?", dao.getId(), field).first();
          db.delete(attr);
        ...
        try {
          DaoAtt d = db.select(daodef).where("parent = ? and name = ?", dao.getId(), field).first();
          d.setValue(value);
          db.update(d);
        } catch (NoMatchFound e) {
          try {
      ...
      changeListeners.inform(dao, field);
    }
  }
  ...
  @Override
  public ObserverReference onChange(String attrib, Runnable run) {
    return changeListeners.register(dao, attrib, run);
  }
  ...
  protected SqlHObjectList getParentList() {
    ...
    if (parent.is(LIST)) {
      try {
        try {
          DaoLsobj lsobj = db.select(DaoLsobj.class).where(" value = ?", dao.getId()).first();
  ...
  private int getIndex() {
    ...
    try {
      try {
        DaoLsobj lsobj = getStore().getDb().select(DaoLsobj.class).where("value = ?", dao.getId()).first();
  ...
  public void triggerDeleteEvent(String name) {
    changeListeners.inform(dao, name);
  }
